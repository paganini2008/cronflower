...
infra (MySQL/PostgreSQL), uncomment a datasource block in `conf/server.properties` — it takes over
...
```
bin/cronflow-server-api-1.0.0-SNAPSHOT.jar
...
- Default H2 is **node-local** (each node has its own store) — fine for a demo. For a real
  shared/sharded cluster, point `conf/server.properties` at a shared MySQL/PostgreSQL reachable
...
`conf/server.properties` (scheduler) and `conf/executor.properties` (executors) are each layered on
top of their jar's packaged best-practice defaults — edit them to tune a running deployment WITHOUT
rebuilding. `conf/server.properties` is also (re)generated by the build from the selected environment:
`DEPLOY_ENV=dev` (default) or `DEPLOY_ENV=prod` picks `mvn -Pdev/-Pprod`, which emits the matching
`application-<env>.properties` as `conf/server.properties` (dev = embedded H2, prod = shared MySQL via
`CRONFLOW_DB_*`). The scheduler file switches the datasource (MySQL/PostgreSQL) or tunes the engine (zone,
...
`conf/server.properties` and both runners propagate that one value everywhere it has to match: the
...
**Login + roles.** The server API requires a bearer token: sign in at `/auth/login` (the console's
default credentials are **admin / admin123**). Accounts are provisioned in the server's XML user store
(`users.xml`, roles `admin` / `scheduler_admin` / `workflow_admin` / `user`); point at your own with
`cronflow.security.users-file=file:conf/users.xml` in `conf/server.properties`, and set a shared
`cronflow.security.jwt.secret` (same on every node). Executor -> scheduler traffic stays open. See the
[server module README](../backend/cronflow-server-api/README.md) for the full auth reference.
...
- Override anything via env, e.g. `MVN=…`, `M2_REPO=…`, `CRONSMITH_REPO=…`, `WEB_PORT=9000`,
  `NG_CONFIG=production`, `DEPLOY_ENV=prod` (build the server for MySQL instead of H2). (There is no
  fixed scheduler port to override — the ports are random.)
